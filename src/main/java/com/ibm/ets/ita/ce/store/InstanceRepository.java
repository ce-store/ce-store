...
public class InstanceRepository {
	...
	public void updateConceptInstanceListsFor(Collection<CeInstance> pInstList) {
		if (pInstList != null) {
			...
			for (CeInstance thisInst : pInstList) {
				//Save this instance into each of the collections for the relevant concepts
				for (CeConcept thisConcept : thisInst.getDirectConcepts()) {
					...
					//TODO: A big performance improvement if we don't save this but instead calculate
					...
					for (CeConcept thisParent : allParents) {
						if (!thisParent.isThing()) {
							//Instances of thing do not need to be stored in this structure (they are available from the allInstances list)
	...
	public void removeInstance(CeInstance pInstance) {
		...
		//Next remove the instance from each of the collections for the relevant concepts
	...
	public void removeAllNonMetamodelInstances(ActionContext pAc) {
		...
		//First reset the allInstances collection - metamodel instances will be added back in during the final stage
		this.allInstances = new ConcurrentHashMap<String, CeInstance>();
		
		//Next iterate through all of the concepts and store a list of the instances of any meta-model concepts
		...
		//Now reset the allInstancesByConcept collection - metamodel instances will be added back in during the final stage
		this.instancesByConcept = new ConcurrentHashMap<CeConcept, CopyOnWriteArrayList<CeInstance>>();
		
		//Finally iterate through each of the metamodel instances and save them back
	...
	protected void deleteAllInstancesForConcept(ActionContext pAc, CeConcept pConcept) {
		//Note that instances are not referenced by any other model entity, so they can be safely deleted here
		//without leaving any references in place.
		//If there are textual references to the instance (i.e. in a property that refers to it by name) then
		//these references must be manually cleaned up separately by the code that calls this method.
		
		//First remove the instances individually from the list of all instances
		...
		//Then remove the unused sentences and sources that may be left as a result
	...
	protected static void deleteExactInstancesForConcept(ActionContext pAc, CeConcept pConcept) {
		//TODO: Implement this
		reportError("Deleting of all exact instances (for concept '" + pConcept.getConceptName() + "') not yet implemented", pAc);
	}
	...
	private void deleteInstanceNoRefs(CeInstance pInst) {
		...
		//First delete from each of the concept level lists for each of the concepts
